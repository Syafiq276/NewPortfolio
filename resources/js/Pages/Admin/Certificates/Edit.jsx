...
export default function Edit({ certificate = {} }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        router.post(route('admin.certificates.update', certificate.id), {
            _method: 'PUT',
            title: data.title,
            issuer: data.issuer,
    ...
    return (
        <AdminLayout>
            <Head title="Edit Certificate - Admin Dashboard" />
            ...
            <div className="max-w-xl mx-auto space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-xl font-bold text-pearl-light mb-1">Edit Certificate</h3>
                        <p className="text-xs text-pearl-muted">Modify certification details, verification links, or issuer logo.</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 rounded-2xl glass-panel border border-lunar-border/30 space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        {certificate.logo_path && (
                            <div className="mb-2">
                                <span className="text-[10px] text-pearl-muted block mb-1">Current Logo Preview:</span>
                                <img
                                    src={certificate.logo_path}
                                    alt="Logo preview"
                                    className="w-16 h-16 object-contain rounded bg-lunar-dark border border-lunar-border/30 p-1"
                                />
                            </div>
                        )}
                        <input
                    ...
                    >
                        <Save className="w-4 h-4" /> Update Certificate
                    </button>
                </form>
            </div>
        </AdminLayout>
